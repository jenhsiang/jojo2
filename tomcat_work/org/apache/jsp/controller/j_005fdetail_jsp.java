 ...
 * Generated at: 2016-06-11 13:16:25 UTC
...
public final class j_005fdetail_jsp extends org.apache.jasper.runtime.HttpJspBase
	...
	public int checkrent(int borrow_id){
		int result = 0;
				DbBean db = new DbBean();
				DateTime nowdate = new DateTime();
				DateTime endtime = nowdate.minusDays(1);
				List whereList = new ArrayList();
				whereList.add(borrow_id);
				...
				whereList2.add(borrow_id);
				whereList2.add(nowdate.toString("yyyy-MM-dd HH:mm:ss"));		
				List duringReserve = db.SelectRSwhere("select * from borrow_record where borrow_id = ? and reserve_date <= ? and reserve_deadline >= ? and borrow_date is null and return_date is null and givefine = 0  ",whereList);
				List overReserve   = db.SelectRSwhere("select * from borrow_record where borrow_id = ? and reserve_deadline <= ? and borrow_date is null and return_date is null and givefine = 0  ",whereList2);
				List duringRent    = db.SelectRSwhere("select * from borrow_record where borrow_id = ? and borrow_date <= ? and borrow_deadline >= ? and  return_date is null and givefine = 0  ",whereList);
				List overRent	   = db.SelectRSwhere("select * from borrow_record where borrow_id = ? and borrow_deadline <= ?  and return_date is null and givefine = 0 ",whereList2);
...
if(request.getParameter("check_fine") != null) check_fine = (String)request.getParameter("check_fine");
  ...
  if( member_id !=0 && book_id !=0 && !book_name.equals("")){
		...
		int check_where = 0;
		String total_sql = " select * ,'" + book_name + "' as book_name from borrow_record  where member_id = ? and book_id = ? ";
		String detail_sql = " select * ,'" + book_name + "' as book_name from borrow_record  where member_id = ? and book_id = ? ";
		...
		for(int i=0;i<ll.size();i++){
			...
			Map map = (Map) ll.get(i); 
				int borrow_state = checkrent((Integer)map.get( "borrow_id"));
				String borrow_string = rentState(borrow_state);
				int real_fine = getFine((Integer)map.get( "givefine"),(Integer)map.get( "fine"),borrow_state,map.get( "reserve_deadline"),map.get( "borrow_deadline")) ;
				...
				out.println("borrow_state:'"+ borrow_state +"',");
